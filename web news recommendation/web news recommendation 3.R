...
for (i in 1:nrow(view_log)){
    P_matrix[view_log[i, 1], view_log[i, 2]] = log(exp(P_matrix[view_log[i, 1], view_log[i, 2]]) + 1)
}
article_P_log = article_info %>%
  select(userID, articleID) %>%
  filter(userID %in% view_log$userID)

for (i in 1:nrow(article_P_log)){
  P_matrix[article_P_log[i, 1], article_P_log[i, 2]] = log(exp(P_matrix[article_P_log[i, 1], article_P_log[i, 2]]) + 3)
}

U_matrix = matrix(rnorm(k * m, sd = init_sd1), nrow = k, ncol = m)
colnames(U_matrix) = unique(view_log$userID)

A_matrix = matrix(rnorm(k * n, sd = init_sd2), nrow = k, ncol = n)
...
top5_long %>% View() # result check

write.csv(top5_long, "web_news_recommendation/result/logP_basic_model_1000_0.01_0.2_creator3.csv", row.names = FALSE)

# exp? ReLU?
# poisson reg으로 변경? log(), logit()
# lambda?
